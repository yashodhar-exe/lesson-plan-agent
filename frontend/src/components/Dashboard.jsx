...
const Dashboard = ({ facultyId, faculty, setCurrentView }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        setLoading(true);
        const workloadRes = await fetch(`${import.meta.env.VITE_API_URL}/api/faculty/${facultyId}/workload`);
    ...
    <>
      ...
      {loading ? (
        ...
        <div className="bg-brand-alert/10 text-brand-alert p-4 rounded text-sm border border-brand-alert/20">
          Error loading data: {error}. Make sure the backend is running on {import.meta.env.VITE_API_URL}.
        </div>
      ) : (
